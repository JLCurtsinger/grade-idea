    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100">
      <div className="mx-auto max-w-2xl p-6">
        ...
        <article id="roast-card" className="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-6 shadow-xl">
          ...
          {!r ? (
            <p className="text-neutral-400">Fetching your roast…</p>
          ) : (
            <>
              <div className="mb-4">
                <div className="text-sm text-neutral-400 mb-1">Zingers</div>
                <ul className="list-disc pl-5 space-y-2">
                  {(r.zingers || []).map((z: string, i: number) => <li key={i}>{z}</li>)}
                </ul>
              </div>
              ...
              <div className="mb-4">
                ...
                <ul className="list-disc pl-5 space-y-2">
                  {(r.insights || []).map((t: string, i: number) => <li key={i}>{t}</li>)}
                </ul>
              </div>

              <div className="text-neutral-300">
                <span className="text-neutral-400">Verdict:</span> {r.verdict}
              </div>
            </>
          )}
        </article>
